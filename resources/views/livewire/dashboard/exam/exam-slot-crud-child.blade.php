    ...
    <x-tall-crud-dialog-modal wire:model="confirmingItemCreation">
        ...
        <div class="grid grid-cols-2 gap-8">
            <div class="mt-4">
                ...
                <x-tall-crud-input class="block mt-1 w-full" type="text" wire:model.defer="item.total_marks" />
                @error('item.total_marks') <x-tall-crud-error-message>{{$message}}</x-tall-crud-error-message> @enderror
            </div>
            <div class="mt-4">
                <x-tall-crud-label>Exam Id</x-tall-crud-label>
                <x-tall-crud-select class="block mt-1 w-full" wire:model.defer="item.exam_id">
                    @foreach($exam as $c)
