<div class="mt-8 min-h-screen">
    ...
    <div class="flex justify-between">
        ...
        <button type="submit" wire:click="$emitTo('dashboard.school.crud-child', 'showCreateForm');" class="text-blue-500">
        ...
        <thead class="bg-secondary text-gray-100 font-bold">
                ...
                <td class="py-2 pl-2" >
                    <div class="flex items-center">
                        ...
                        <x-tall-crud-sort-icon sortField="address" :sort-by="$sortBy" :sort-asc="$sortAsc" />
                    </div>
                </td>
                <td class="py-2 pl-2" >Email</td>
                <td class="py-2 pl-2" >Phone</td>
                <td class="py-2 pl-2" >Semester Id</td>
                <td class="py-2 pl-2" >Academic Year Id</td>
            ...
            @foreach($results as $result)
                <tr class="hover:bg-blue-300 {{ ($loop->even ) ? "bg-blue-100" : ""}}">
                    ...
                    <td class="py-3 pl-2 capitalize" >{{ $result->address }}</td>
                    <td class="py-3 pl-2 capitalize" >{{ $result->email }}</td>
                    <td class="py-3 pl-2 capitalize" >{{ $result->phone }}</td>
                    <td class="py-3 pl-2 capitalize" >{{ $result->semester_id }}</td>
                    <td class="py-3 pl-2 capitalize" >{{ $result->academic_year_id }}</td>
                    <td class="py-3 pl-2 capitalize" >{{ $result->code }}</td>
                    <td class="py-3 pl-2 capitalize" >
                        <button type="submit" wire:click="$emitTo('dashboard.school.crud-child', 'showEditForm', {{ $result->id}});" class="text-green-500">
                            <x-tall-crud-icon-edit />
                        </button>
                        <button type="submit" wire:click="$emitTo('dashboard.school.crud-child', 'showDeleteForm', {{ $result->id}});" class="text-red-500">
